...
io.on('connection', (socket) => {
  ...
  socket.on('join_room', ({ roomCode, playerName }) => {
    ...
    if (room.state && room.state.started) {
      socket.emit('error_msg', 'Играта вече е започнала!');
      return;
    }
    if (room.players.length >= 4) {
      socket.emit('error_msg', 'Стаята е пълна (макс. 4 играчи)!');
      return;
    }

    room.players.push({ name: playerName, socketId: socket.id });
    room.sockets[socket.id] = room.players.length - 1; // player index
  ...
  socket.on('start_game', ({ roomCode }) => {
    ...
    if (room.players.length < 2) {
      socket.emit('error_msg', 'Трябват поне 2 играчи!');
      return;
    }
    room.state = createInitialState(room.players.map(p => p.name));
  ...
  socket.on('roll_dice', ({ roomCode }) => {
    ...
    if (pl.pos < oldPos) {
      pl.money += 200;
      state.log.unshift(`🏁 ${pl.name} мина СТАРТ — +200 MC!`);
    }
...
function processCell(state, pl, d1, d2) {
  ...
  if (cell.type === 'prop' || cell.type === 'railroad' || cell.type === 'utility') {
    ...
    if (!owner) {
      ...
      else { state.log.unshift(`${pl.name} няма достатъчно MC`); return 'next'; }
    } else if (owner.id !== pl.id) {
      let rent = 0;
      if (cell.type === 'prop') rent = cell.rent[0];
      else if (cell.type === 'railroad') {
        const o = owner.props.filter(p => BOARD[p] && BOARD[p].type === 'railroad').length;
        rent = 25 * Math.pow(2, o - 1);
      } else if (cell.type === 'utility') {
        const dice = d1 + d2;
        const numC = pl.props.length || 1;
        const ownsBoth = owner.props.includes(12) && owner.props.includes(28);
        rent = Math.round(dice * Math.pow(numC, 2));
        if (ownsBoth) rent *= 2;
      }
      ...
      state.log.unshift(`${pl.name} кацна на свой бизнес`);
      return 'next';
    }
  } else if (cell.type === 'tax') {
    if (cell.fee < 0) {
      pl.money += (-cell.fee);
      state.log.unshift(`⛏️ ${pl.name} спечели ${-cell.fee} MC от Крипто мина!`);
    } else {
      const fee = Math.min(cell.fee, pl.money);
      pl.money -= fee;
      state.log.unshift(`🏆 ${pl.name} плати Хакатон ${fee} MC`);
    }
    return 'next';
  } else if (cell.type === 'chance' || cell.type === 'chest') {
    const cards = [
      {txt:'💡 Стартъп идея! +150 MC', val:150},
      {txt:'🐛 Бъг в кода! -100 MC', val:-100},
      {txt:'📈 Инвеститор! +200 MC', val:200},
      {txt:'🔒 Кибератака! -150 MC', val:-150},
      {txt:'🎓 Спечели хакатон! +100 MC', val:100},
      {txt:'💾 Срив на сървъра! -50 MC', val:-50},
      {txt:'🚀 Вирусен продукт! +50 MC', val:50},
      {txt:'📱 Вирусна публикация! +100 MC', val:100},
      {txt:'⚡ Хардуерна авария! -100 MC', val:-100},
      {txt:'🌐 Домейнът изтече! -50 MC', val:-50},
    ];
    const card = cards[Math.floor(Math.random() * cards.length)];
    if (card.val > 0) pl.money += card.val;
    else { const l = Math.min(-card.val, pl.money); pl.money -= l; }
    state.log.unshift(`🃏 ${pl.name}: ${card.txt}`);
    return 'next';
  } else if (cell.type === 'corner') {
    if (cell.pos === 30) {
      pl.pos = 10;
      state.log.unshift(`💥 ${pl.name} — СРИВ В СИСТЕМАТА! В ИТ Поддръжка.`);
    }
    return 'next';
  }
  return 'next';
